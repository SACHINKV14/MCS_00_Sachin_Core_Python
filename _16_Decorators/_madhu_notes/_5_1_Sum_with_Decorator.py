...
'''
'''
a = valiate_numbers(sum)  # reference of wrapper
a(params)
def validate_numbers(func): # func = sum
    def wrapper():  # *args = n1 n2
        if args[0] >= 0 or args[1] >= 0:
            res = sum(*args)  # sum(n1, n2)
            return res
        else:
        raise NumberException("Please enter positive number")
    return wrapper
...
def ec_func(func):
    def wrapper():
        print('before accesssing outer fucntion')
        outer = func()
        print('after accessing outer function')
        return 'process is one'
    # print('wrapper function aress :', wrapper)
    return wrapper
@ec_func
def outer_func():
    print('im in the outer function')

# print('without ecorator function..........')
outer_func()
# print('with ecorator function............')
# result = ec_func(outer_func) #
# # print('wrapper funciton referencee',result)
# result()


"""
result = parent()  chil function reference
result()
"""