...
string array denoting the composition of every group s has been divided into, using the above procedure.
Example 1:
...
def myfu(str1,split,fill):
    list1=[]
    for x in range(0,len(str1),k):
        list1.append(str1[x:x+k])
    if len(list1[-1])==k:
        print(list1)
    else:
        a=k-len(list1[-1])
