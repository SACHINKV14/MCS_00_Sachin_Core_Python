...
def adminsf(request):
    if request.method == "POST":
        ...
        adminuser = Admins.objects.filter(adminusername=adminun).first()
        adminlogins = request.POST.get("adminlogin")

        aupwords = adminuser.adminpassword
        auun = adminuser.adminusername

        if adminlogins:
            if adminun == auun and adminpw == aupwords:
                return redirect("adminf")

            else:
                messages.info(request, "Check your UserName & Password ")
